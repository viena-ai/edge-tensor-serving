...
class ModelServer:
    def __init__(self, model_info_list):
        ...
        #TODO Load TF Config from tf-config file
        ...
        for model_info in model_info_list:
            ...
            model_values_dict = OrderedDict()
            input_params_dict = OrderedDict()
            output_params_dict = OrderedDict()
            op_to_restore = []
            ...
            #TODO we need to check what tag to call. is it always 'serve'?
            loaded = tf.saved_model.loader.load(session, [tag_set], model_info_path)

            graph = tf.get_default_graph()
            session.run(tf.global_variables_initializer())

            model_values_dict['session'] = session
            model_values_dict['graph'] = graph
            ...
            for input_param in input_params:
                 input_params_dict[input_param] = model_values_dict['graph'].get_tensor_by_name(input_param)

            # output params
            for output_param in output_params:
                output_params_dict[output_param] = model_values_dict['graph'].get_tensor_by_name(output_param)
                op_to_restore.append(output_params_dict[output_param])

            model_values_dict['input_params'] = input_params_dict
            model_values_dict['output_params'] = output_params_dict
            model_values_dict['op_to_restore'] = op_to_restore

            self.loaded_models[model_info_name] = model_values_dict
            ...
            input params
            self.x_tensor_1 = self.graph_1.get_tensor_by_name("input_image:0")
            self.x_tensor_2 = self.graph_1.get_tensor_by_name("input_image_2:0")
            
            output params
            self.op_to_restore1 = self.graph_1.get_tensor_by_name("detection_1/Reshape_1:0")
            self.op_to_restore2 = self.graph_1.get_tensor_by_name("detection_2/Reshape_1:0")
    ...
    def infer_2(self, input_values):
        x_test = input_values.astype(np.float32)

        feed_dict = {self.x_tensor_infer: x_test}
        opt = self.sess_bruise.run(self.infer_model, feed_dict)
